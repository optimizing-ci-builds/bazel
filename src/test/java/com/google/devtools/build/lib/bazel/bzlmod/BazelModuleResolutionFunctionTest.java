...
public class BazelModuleResolutionFunctionTest extends FoundationTestCase {
  ...
  public void setup() throws Exception {
    ...
    evaluator =
        new InMemoryMemoizingEvaluator(
            ImmutableMap.<SkyFunctionName, SkyFunction>builder()
                .put(
                    FileValue.FILE, new FileFunction(packageLocator, directories))
                .put(
                    FileStateKey.FILE_STATE,
                      new FileStateFunction( Suppliers.ofInstance(
                          new TimestampGranularityMonitor(BlazeClock.instance())),
                          SyscallCache.NO_CACHE, externalFilesHelper))
                .put(
                    SkyFunctions.MODULE_FILE,
                    new ModuleFileFunction(registryFactory, rootDirectory, ImmutableMap.of()))
                .put(
                    SkyFunctions.PRECOMPUTED, new PrecomputedFunction())
                .put(
    ...
    BazelModuleResolutionFunction.CHECK_DIRECT_DEPENDENCIES.set(differencer, CheckDirectDepsMode.OFF);
  }
  ...
  private void setupModulesForCompatability() throws IOException {
    /* Root depends on a and b
        The only versions that would work with this compatability constrains is:
        >5.1.2 && <5.1.4 Ex: 5.1.3rc44, 5.1.3, 5.1.4-pre22.44
     */
    scratch.file(
        rootDirectory.getRelative("MODULE.bazel").getPathString(),
